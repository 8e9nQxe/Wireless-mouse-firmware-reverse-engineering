...
### 0.4 更新記錄（更新時間：2026-04-30，新韌體：`deverV0.1.4_20260430.bin`）

本節用於記錄相對於上一版樣本的增量觀察。先把結論說在前面：這次更新確實補了不少功能，但整體氣質仍然很符合 CHAOS 家族一貫的低成本自研風格，也就是「先把能跑的東西全塞進去，再慢慢想辦法別讓它當場散架」。從韌體工程角度直接評價，這版實作仍然不行。它確實比舊版多了功能，鏈路也更完整，但這些增量主要是透過繼續堆疊模組、狀態機和相容分支實現的，並沒有伴隨相應的結構收斂。當前樣本仍然表現出明顯的工程品質問題：全域狀態依賴沉重，歷史相容負擔持續累積，命名語義與實際行為多處錯位，系統行為對補丁式修補和路徑分叉有較強依賴。換句話說，這次更新提升的是「可交付表象」，不是「實作品質」；與主流滑鼠韌體水準差距依然很大。

#### 0.4.1 最顯眼的變化：從「無線主路徑韌體」擴成了「混合鏈路平台」

從功能邊界看，新版本已經不再是單一無線主路徑韌體，而是擴成了同時覆蓋無線、有線與充電場景的混合鏈路平台：

- 啟動日誌直接出現 `USB mouse APP starting...`
- 架構描述字串直接寫著 `modular APP + low-latency wired HID`
- 韌體會在 `WIRELESS / WIRED_HOST / CHARGER_ONLY` 三種鏈路狀態之間切換
- 有線側不只掛了 mouse HID，還額外帶出 keyboard / config / media 三類介面

但從工程品質角度看，這一步屬於功能域外擴，不屬於架構收斂。韌體只是把更多鏈路和更多裝置角色疊加到原有實作之上，並沒有建立一個足夠清晰的統一控制面。鏈路狀態更多了，服務對象更多了，初始化與切換路徑也更長了，可系統的核心組織方式依然是全域狀態加分支調度。這代表平台能力確實增強了，但實作複雜度上升得更快，結構品質並沒有同步改善。

#### 0.4.2 配置協議補全了，但也更暴露「邊做邊補」的痕跡

和舊版相比，這一版明顯擴寫了配置協議，把若干原先語義模糊、或者根本不存在的配置入口補齊了：

- `0x14` 不再是「特殊硬體動作，未完全確認」，而是明確落成 `SET_DFU`
- 新增 `0x24` 新式恢復出廠，同時保留 `0xFF` 舊式恢復出廠相容入口
- 新增 `0x25` 配對令牌寫入
- 新增 `0x27` 充電狀態查詢
- 配置回包除了無線 ACK 路徑，還可以走獨立的 `cfg_hid` 佇列

這說明協議面已經從「夠用」走向「補全」，但其演進方式仍然是典型的累加式修補，而不是受控重構。新命令持續增加，舊命令持續保留，配置回包路徑持續分叉，協議表面更完整了，內部語義卻沒有明顯變得更統一。對成熟韌體來說，協議擴展通常會伴隨版本邊界、語義歸一與歷史負擔收斂；而當前樣本更像是在既有協議表面不斷貼補丁。這種做法短期內能解決產品需求，長期看只會提高維護成本與行為歧義。

#### 0.4.3 `feel` 與「輔助壓槍」沒被砍，而是被塞進了公共運動處理層

這一點是本次更新裡最重要的實作層變化之一：舊版分散在無線主路徑中的手感邏輯，開始被收編到公共運動處理層中。

- `feel` 仍然保留「先按 `min(26000 / dpi, feel)` 放大內部 CPI，再做帶餘數保留的量化回縮」這條核心算法
- 「輔助壓槍」仍然保留 5 點滑動平均，不是換了新花樣，只是換了落點
- 兩者都不再只掛在舊式無線主路徑裡，而是被收編進獨立的 `motion_processor`
- wired / wireless 兩條鏈路都複用了這一層運動處理

這比舊版到處散落的路徑內嵌實作要好一些，但還不能算真正完成了解耦。當前 `motion_processor` 更像是對舊算法做了集中封裝，而不是圍繞運動處理重新定義了一套乾淨的資料邊界。配置狀態、模式位、鏈路差異與輸出行為之間仍然存在明顯的全域耦合，控制面與資料面也沒有被徹底分開。換句話說，它解決的是「程式碼別到處抄」的問題，沒有解決「邏輯應如何被穩定建模」的問題。

#### 0.4.4 PAW3395 profile 語義發生了偏移，命名比以前更不老實

舊版報告裡，低功耗 / 高效能 / 競技 / 超頻四套 profile 還能基本對應成一組相對穩定的模式語義。新版本裡，能直接確認的主檔變成了：
...
這意味著 profile 語義發生了明顯漂移：舊版裡相對清楚的 `competition profile` 在當前樣本裡不再以獨立形態出現，取而代之的是一個 `legacy USB profile`。從工程審查看，這不是好訊號。成熟韌體的模式定義通常應當保持穩定映射：UI 名稱、配置位語義、底層指令碼與執行時行為應該盡量單調一致。而當前樣本表現出的，是命名體系、歷史調校與當前實作之間沒有被重新整理，只是繼續疊在一起執行。這會直接降低可維護性，也會增加後續逆向和驗證成本。

#### 0.4.5 新增了一整圈配套狀態機：配對、充電、DFU、鏈路恢復

這一版明顯把外圍流程補齊了，而且是以增加更多狀態機與流程分支的方式完成的：

- 有正式的配對狀態進入、逾時、成功、失敗流程
- 配對成功後會保存 `rand/model` 位元組並刷新無線基地址
- 有充電檢測與 `CHARGER_ONLY` 模式切換
- 有 DFU 入口
- 有 USB session 啟停、bus stalled 後重啟、掉電後停止等恢復邏輯

這說明它已經開始覆蓋量產產品一定會遇到的邊角狀態，但實作方式仍然偏粗放。它不是透過統一的 session 管理、事件模型或生命週期抽象來管理這些外圍流程，而是繼續增加進入、退出、失敗、重試、恢復等顯式路徑，把行為分散到更多局部狀態與條件分支裡。結果是產品表現更完整了，但狀態空間也顯著膨脹。就工程品質而言，這屬於「把場景兜住了」，不屬於「把系統理順了」。

#### 0.4.6 某些舊問題依舊沒解決，只是被新框架蓋住了

最典型的就是 `motion sync` 這類位。新版本裡，它仍然表現為一個被保留但未落實到明確行為的歷史佔位符：

- 標誌位還能存
- 配置面還能帶
- 但沒有看到它進入明確的實際運動處理主鏈

從工程視角看，這類殘留位不是無害細節，而是實作品質不足的直接證據。它說明系統表面能力與實際落地能力之間仍然存在斷層：介面可以先暴露，配置可以先保留，但行為定義沒有閉合，呼叫鏈也沒有補全。這種「先掛語義、後補實作，最後可能根本沒補完」的模式，在低品質韌體裡很常見；它會持續製造歧義、誤判與歷史債務，也是這類樣本長期擺脫不了的工程問題之一。

#### 0.4.7 對這次更新的總體評價

這次更新的本質，不是一次實作品質提升，而是一次明顯的功能外擴。韌體把 wired USB、config HID、配對、充電、DFU 和鏈路恢復這些產品側必須補齊的東西都補上了，因此「能做的事」確實比上一版更多；但從工程角度看，這次改動沒有把系統做得更規整，反而進一步放大了原本就存在的結構問題。它仍然主要依賴全域狀態驅動行為，仍然用不斷增加的狀態機和相容入口去覆蓋新場景，仍然缺少足夠清晰的模組邊界與一致的控制抽象，因此整體實作品質並沒有隨著功能數量同步上升。

更直接地說，這一版依然是一份品質偏低的韌體樣本：

- 它解決的是「功能缺口」問題，不是「結構失控」問題。
- 它增加的是路徑和分支，不是可驗證、可收斂的統一機制。
- 它把舊邏輯封裝進了新模組，但沒有同步清理舊的全域依賴和歷史語義包袱。
- 它在產品表現上比舊版完整，但在實作層面仍然帶著明顯的補丁化、拼接化和低一致性特徵。

因此，對這次更新最準確的評價不是「韌體成熟了」，而是「韌體變大了」。它比上一版更像一個能交付的產品韌體，但離一份設計清晰、邊界明確、狀態可控、技術債受約束的成熟實作還有明顯距離；如果把它放到主流滑鼠韌體的工程標準下看，與主流滑鼠韌體水準差距依然很大。
