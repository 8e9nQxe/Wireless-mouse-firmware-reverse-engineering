...
### 0.4 更新记录（更新时间：2026-04-30，新固件：`deverV0.1.4_20260430.bin`）

本节用于记录相对于上一版样本的增量观察。先把结论说在前面：这次更新确实补了不少功能，但整体气质仍然很符合 CHAOS 家族一贯的低成本自研风格，也就是“先把能跑的东西全塞进去，再慢慢想办法别让它当场散架”。从固件工程角度直接评价，这版实现仍然不行。它确实比旧版多了功能，链路也更完整，但这些增量主要是通过继续堆叠模块、状态机和兼容分支实现的，并没有伴随相应的结构收敛。当前样本仍然表现出明显的工程质量问题：全局状态依赖沉重，历史兼容负担持续累积，命名语义与实际行为多处错位，系统行为对补丁式修补和路径分叉有较强依赖。换句话说，这次更新提升的是“可交付表象”，不是“实现质量”；与主流鼠标固件水平差距依然很大。

#### 0.4.1 最显眼的变化：从“无线主路径固件”扩成了“混合链路平台”

从功能边界看，新版本已经不再是单一无线主路径固件，而是扩成了同时覆盖无线、有线和充电场景的混合链路平台：

- 启动日志直接出现 `USB mouse APP starting...`
- 架构描述字符串直接写着 `modular APP + low-latency wired HID`
- 固件会在 `WIRELESS / WIRED_HOST / CHARGER_ONLY` 三种链路状态之间切换
- 有线侧不只挂了 mouse HID，还额外挂了 keyboard / config / media 三类接口

但从工程质量角度看，这一步属于功能域外扩，不属于架构收敛。固件只是把更多链路和更多设备角色叠加到原有实现之上，并没有建立一个足够清晰的统一控制面。链路状态更多了，服务对象更多了，初始化和切换路径也更长了，可系统的核心组织方式依然是全局状态加分支调度。这意味着平台能力确实增强了，但实现复杂度上升得更快，结构质量并没有同步改善。

#### 0.4.2 配置协议补全了，但也更暴露“边做边补”的痕迹

和旧版相比，这一版明显扩写了配置协议，把若干原先语义模糊、或者根本不存在的配置入口补齐了：

- `0x14` 不再是“特殊硬件动作，未完全确认”，而是明确落成 `SET_DFU`
- 新增 `0x24` 新式恢复出厂，同时保留 `0xFF` 旧式恢复出厂兼容入口
- 新增 `0x25` 配对令牌写入
- 新增 `0x27` 充电状态查询
- 配置回包除了无线 ACK 路径，还可以走独立的 `cfg_hid` 队列

这说明协议面已经从“够用”走向“补全”，但其演进方式仍然是典型的累加式修补，而不是受控重构。新命令继续增加，旧命令继续保留，配置回包路径继续分叉，协议表面更完整了，内部语义却没有明显变得更统一。对成熟固件来说，协议扩展通常会伴随版本边界、语义归一和历史负担收敛；而当前样本更像是在既有协议表面不断贴补丁。这种做法短期内能解决产品需求，长期看只会提高维护成本和行为歧义。

#### 0.4.3 `feel` 和“辅助压枪”没被砍，而是被塞进了公共运动处理层

这一点是本次更新里最重要的实现层变化之一：旧版分散在无线主路径中的手感逻辑，开始被收编到公共运动处理层中。

- `feel` 仍然保留“先按 `min(26000 / dpi, feel)` 放大内部 CPI，再做带余数保留的量化回缩”这条核心算法
- “辅助压枪”仍然保留 5 点滑动平均，不是换了新花样，只是换了落点
- 两者都不再只挂在旧式无线主路径里，而是被收编进了独立的 `motion_processor`
- wired / wireless 两条链路都复用了这一层运动处理

这比旧版到处散落的路径内嵌实现要好一些，但还不能算真正完成了解耦。当前 `motion_processor` 更像是对旧算法做了集中封装，而不是围绕运动处理重新定义了一套干净的数据边界。配置状态、模式位、链路差异和输出行为之间仍然存在明显的全局耦合，控制面和数据面也没有被彻底分开。换句话说，它解决的是“代码别到处抄”的问题，没有解决“逻辑应如何被稳定建模”的问题。

#### 0.4.4 PAW3395 profile 语义发生了偏移，命名比以前更不老实

旧版报告里，低功耗 / 高性能 / 竞技 / 超频四套 profile 还能基本对应成一组相对稳定的模式语义。新版本里，能直接确认的主档变成了：
...
这意味着 profile 语义发生了明显漂移：旧版里相对清楚的 `competition profile` 在当前样本里不再以独立形态出现，取而代之的是一个 `legacy USB profile`。从工程审查看，这不是好信号。成熟固件的模式定义通常应当保持稳定映射：UI 名称、配置位语义、底层脚本和运行时行为应该尽量单调一致。而当前样本表现出的，是命名体系、历史调校和当前实现之间没有被重新整理，只是继续叠在一起运行。这会直接降低可维护性，也会增加后续逆向和验证成本。

#### 0.4.5 新增了一整圈配套状态机：配对、充电、DFU、链路恢复

这一版明显把外围流程补齐了，并且是以增加更多状态机和流程分支的方式完成的：

- 有正式的配对状态进入、超时、成功、失败流程
- 配对成功后会保存 `rand/model` 字节并刷新无线基地址
- 有充电检测与 `CHARGER_ONLY` 模式切换
- 有 DFU 入口
- 有 USB session 启停、bus stalled 后重启、掉电后停止等恢复逻辑

这说明它已经开始覆盖量产产品一定会遇到的边角状态，但实现方式仍然偏粗放。它不是通过统一的会话管理、事件模型或生命周期抽象来管理这些外围流程，而是继续增加进入、退出、失败、重试、恢复等显式路径，把行为分散到更多局部状态和条件分支里。结果是产品表现更完整了，但状态空间也显著膨胀。就工程质量而言，这属于“把场景兜住了”，不属于“把系统理顺了”。

#### 0.4.6 某些旧问题依旧没解决，只是被新框架盖住了

最典型的就是 `motion sync` 这类位。新版本里，它仍然表现为一个被保留但未落实到明确行为的历史占位符：

- 标志位还能存
- 配置面还能带
- 但没有看到它进入明确的实际运动处理主链

从工程视角看，这类残留位不是无害细节，而是实现质量不足的直接证据。它说明系统表面能力和实际落地能力之间仍然存在断层：接口可以先暴露，配置可以先保留，但行为定义没有闭合，调用链也没有补全。这种“先挂语义、后补实现，最后可能根本没补完”的模式，在低质量固件里很常见；它会持续制造歧义、误判和历史债务，也是这类样本长期摆脱不了的工程问题之一。

#### 0.4.7 对这次更新的总体评价

这次更新的本质，不是一次实现质量提升，而是一次明显的功能外扩。固件把 wired USB、config HID、配对、充电、DFU 和链路恢复这些产品侧必须补齐的东西都补上了，因此“能做的事”确实比上一版更多；但从工程角度看，这次改动没有把系统做得更规整，反而进一步放大了原本就存在的结构问题。它仍然主要依赖全局状态驱动行为，仍然用不断增加的状态机和兼容入口去覆盖新场景，仍然缺少足够清晰的模块边界与一致的控制抽象，因此整体实现质量并没有随着功能数量同步上升。

更直接地说，这一版依然是一份质量偏低的固件样本：

- 它解决的是“功能缺口”问题，不是“结构失控”问题。
- 它增加的是路径和分支，不是可验证、可收敛的统一机制。
- 它把旧逻辑封装进了新模块，但没有同步清理旧的全局依赖和历史语义包袱。
- 它在产品表现上比旧版完整，但在实现层面仍然带着明显的补丁化、拼接化和低一致性特征。

因此，对这次更新最准确的评价不是“固件成熟了”，而是“固件变大了”。它比上一版更像一个能交付的产品固件，但离一份设计清晰、边界明确、状态可控、技术债受约束的成熟实现还有明显距离；如果把它放到主流鼠标固件的工程标准下看，与主流鼠标固件水平差距依然很大。
